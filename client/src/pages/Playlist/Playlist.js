      ...
      <div style={{position: 'absolute', left: 0, top: 130}}>
        ...
        <div>
          ...
          {props.savedTracks.length ? (
            <ul 
            onRowSelection={props.handlePlaylistRowSelection} 
            style={{backgroundColor: '#F7F9FF', padding: 10, border: '1px solid #5A66E3', marginTop: '-21px', width: 400, maxHeight: 518, overflow: 'scroll', float: 'left'}}>
          
            {props.savedTracks.map((track, index) => {
              return (
                <li key={track._id}  
                // {props.selectedPlaylistTrack=playlistRowIsSelected(index)} 
                style={{listStyleType: 'none', fontFamily: 'Montserrat'}}>
                  <IconButton 
